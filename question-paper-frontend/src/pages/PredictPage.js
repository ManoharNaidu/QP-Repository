...
const PredictPage = () => {
  ...
  const handlePredict = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        err.response?.data?.message || "Failed to generate prediction. Please try again later."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-background text-on-surface font-body antialiased py-24 px-6 md:px-12">
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          >
            Enter a course code to analyze historical paper patterns and predict high-probability questions for your next exam.
          </motion.p>
        </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {prediction.frequentTopics.map((item, idx) => (
                    <div key={idx} className="bg-surface border border-outline p-6 rounded-2xl hover:border-primary/30 transition-all">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="text-lg font-bold">{item.topic}</h4>
                        <span className={`px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider ${
                          item.importance === 'High' ? 'bg-error-container text-on-error-container' : 
                          item.importance === 'Medium' ? 'bg-warning-container text-on-warning-container' : 'bg-primary-container text-on-primary-container'
                        }`}>
                          {item.importance} Priority
                        </span>
                      </div>
                      <p className="text-on-surface-variant text-sm italic">"{item.reason}"</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                   {prediction.predictedQuestions.map((item, idx) => (
                    <div key={idx} className="bg-surface border border-outline p-6 rounded-2xl border-l-4 border-l-primary hover:shadow-lg transition-all">
            ...
            <div className="bg-surface border border-outline p-10 rounded-3xl space-y-6">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {prediction.tips.map((tip, idx) => (
                  <li key={idx} className="flex gap-4 items-start p-4 bg-background rounded-xl border border-outline/50">
        ...
        {!prediction && !loading && !error && (
          <div className="text-center py-20 opacity-30 grayscale pointer-events-none">
             <span className="material-symbols-outlined text-9xl mb-4">data_exploration</span>
             <p className="text-xl font-bold">Awaiting historical data analysis...</p>
          </div>
        )}
      </div>
    </div>

...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-50 bg-background flex items-center justify-between px-6 md:px-12 py-4 tracking-tight transition-all duration-300 ${
          scrolled ? "border-b border-outline shadow-sm" : ""
        }`}
      >
        ...
        <div className="hidden md:flex items-center space-x-8 ml-auto mr-10">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
        ...
        {/* Action Icons (Theme & Mobile Menu) */}
        ...
        {menuOpen && (
          <>
            ...
            >
              ...
              <p className="font-label text-xs uppercase tracking-widest text-on-surface-variant mb-4 ml-1">
                Navigation
              </p>
